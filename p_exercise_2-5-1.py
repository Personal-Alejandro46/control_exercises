...
# Proportional Control Analysis

# Does the proportional control stabilize the link angles to the desired values?

# The system stabilizes (oscillates), but it does not converge asymptotically to the desired setpoint.
# This means that although the link angles tend to remain near the target values after some oscillations, the steady-state error remains — the system never perfectly reaches the desired positions.

# Best Combination of KP1 and KP2

# Even though no proportional gain combination achieves full asymptotic stabilization, qualitatively the best performance is obtained when:

# KP1 = 2 and KP2 = 2

# because:

# When KP2 = 0.2, the system response is too slow (low gain → underdamped and sluggish).

# When KP2 = 20, the system response becomes too oscillatory (high gain → overshoot and instability).

# Thus, KP1 = KP2 = 2 provides the most balanced behavior, with moderate rise time and acceptable oscillation amplitude.

# Summary:

# KP2 = 0.2 → Very slow, under-responsive.
# KP2 = 2 → Balanced, best qualitative result.
# KP2 = 20 → Highly oscillatory, over-responsive / unstable.

# What happens when KP2 increases?

# Qualitatively, the link angles oscillate more, but at the same time, the link angles react faster to the presence of an error.