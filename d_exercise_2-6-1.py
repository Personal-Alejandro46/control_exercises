...
# Will the derivative control stabilize the link angles to the desired values? (Answer this question before and after the implementation)

# No. The link angles will remain constant at their initial values because both angular velocities of the links are zero.
# A qualitative conclusion from this question is that if the link angles and setpoint angles become constant at some point (zero angular velocity), the derivative control will have no effect.

# When the initial angular velocities are changed to
# θ̇₁(0) = 2 [deg/s], θ̇₂(0) = −2 [deg/s],
# the behavior changes as follows:

# No matter whether the system stabilizes or not, which is the best combination for KD₁ and KD₂?

# Like in the proportional control, the medium combination
# KD₁ = 5.0 and KD₂ = 5.0
# gives the best results.
# KD₁ = 50.0 and KD₂ = 50.0 are too fast and generate a steady-state error below the setpoint,
# while KD₁ = 0.5 and KD₂ = 0.5 are too slow and generate a steady-state error above the setpoint.

# What happens when KD₁ and KD₂ increase?

# When KD₁ and KD₂ increase, the derivative control becomes faster in correcting the error.
# However, unlike proportional control, this faster response occurs because of the rate of change of the angles, not the difference between their values.